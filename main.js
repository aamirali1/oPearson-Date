(function() {
  ...
  app = angular.module("oPearsonDate", []);

  app.provider("oPearsonDateDefaults", function() {
    return {
      options: {
        dateFormat: 'M/d/yyyy',
        timeFormat: 'h:mm a',
  ...
  app.directive("pearsonDatepicker", [
    'oPearsonDateDefaults', '$filter', '$sce', function(oPearsonDateDefaults, $filter, $sce) {
      return {
        restrict: "E",
        require: "?ngModel",
        scope: {
        ...
        link: function(scope, element, attrs, ngModelCtrl) {
          ...
          setConfigOptions = function() {
            var key, value;
            for (key in oPearsonDateDefaults) {
              value = oPearsonDateDefaults[key];
              if (key.match(/[Hh]tml/)) {
                scope[key] = $sce.trustAsHtml(oPearsonDateDefaults[key] || "");
              } else if (!scope[key] && attrs[key]) {
                scope[key] = attrs[key];
              } else if (!scope[key]) {
                scope[key] = oPearsonDateDefaults[key];
              }
            }
          ...
          parseDateString = oPearsonDateDefaults.parseDateFunction;
          ...
          scope.$watch('calendarShown', function(newVal, oldVal) {
            var dateInput;
            if (newVal) {
              dateInput = angular.element(element[0].querySelector(".pearsondate-date-input"))[0];
        ...
        template: "<div class='pearsondate'>\n  <a href='' ng-focus='toggleCalendar()' ng-click='toggleCalendar()' class='pearsondate-button' title='{{hoverText}}'><div ng-hide='iconClass' ng-bind-html='buttonIconHtml'></div>{{mainButtonStr}}</a>\n  <div class='pearsondate-popup' ng-class='{open: calendarShown}'>\n    <a href='' tabindex='-1' class='pearsondate-close' ng-click='toggleCalendar()'><div ng-bind-html='closeButtonHtml'></div></a>\n    <div class='pearsondate-text-inputs'>\n      <div class='pearsondate-input-wrapper'>\n        <label>Date</label>\n        <input class='pearsondate-date-input' ng-class=\"{'ng-invalid': inputDateErr}\" name='inputDate' type='text' ng-model='inputDate' placeholder='{{ datePlaceholder }}' ng-enter=\"selectDateFromInput(true)\" ng-blur=\"selectDateFromInput(false)\" on-tab='onDateInputTab()' />\n      </div>\n      <div class='pearsondate-input-wrapper' ng-hide='disableTimepicker'>\n        <label>Time</label>\n        <input class='pearsondate-time-input' ng-class=\"{'ng-invalid': inputTimeErr}\" name='inputTime' type='text' ng-model='inputTime' placeholder='{{ timePlaceholder }}' ng-enter=\"selectDateFromInput(true)\" ng-blur=\"selectDateFromInput(false)\" on-tab='onTimeInputTab()'>\n      </div>\n    </div>\n    <div class='pearsondate-calendar-header'>\n      <a href='' class='pearsondate-prev-month pearsondate-action-link' tabindex='-1' ng-click='prevMonth()'><div ng-bind-html='prevLinkHtml'></div></a>\n      <span class='pearsondate-month'>{{calendarDate | date:'MMMM yyyy'}}</span>\n      <a href='' class='pearsondate-next-month pearsondate-action-link' ng-click='nextMonth()' tabindex='-1' ><div ng-bind-html='nextLinkHtml'></div></a>\n    </div>\n    <table class='pearsondate-calendar'>\n      <thead>\n        <tr>\n          <th ng-repeat='day in dayAbbreviations'>{{day}}</th>\n        </tr>\n      </thead>\n      <tbody>\n        <tr ng-repeat='week in weeks'>\n          <td ng-mousedown='selectDate(day.date, true, true)' ng-click='$event.preventDefault()' ng-class='{\"other-month\": day.other, \"disabled-date\": day.disabled, \"selected\": day.selected, \"is-today\": day.today}' ng-repeat='day in week'>{{day.date | date:'d'}}</td>\n        </tr>\n      </tbody>\n    </table>\n    <div class='pearsondate-popup-footer'>\n      <a href='' class='pearsondate-clear' tabindex='-1' ng-hide='disableClearButton' ng-click='clear()'>Clear</a>\n    </div>\n  </div>\n</div>"
      };
    }
  ]);
